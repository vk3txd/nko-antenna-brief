...
- _Common Mode_ Attempts tp suppress feedline current  
...
- _Common Mode current_
  ...
  - _Mechanism_ Unequal currents attempt to flow, but the balun presents high impedance to common-mode paths
...
- _Loss_ Core loss increases with imbalance and impedance match  
...
- _Definition_ A voltage balun (Ruthroff) enforces equal voltages  
...
- _Common Mode_ Allows common-mode current  
...
- _Mismatch Loss_ Loss distributed between core and feedline  
...
- _In NKO Context_ Does not provide the same phase relationship not the same current drive to the vertical coax

---

## 4:1 UnUn (Autotransformer)

- _Definition_ Unbalanced-to-unbalanced impedance transformer  
...
- _Mismatch Loss_ Lower core loss but higher system-level current effects  
...
- _Interaction_ Part of total radiator system  
- _Failure Mode_ Loss is driven mainly by impedance mismatch and load current, producing heating
- _In NKO Context_ Enables in-phase current addition  
...
| _Installation Sensitivity_ | Moderate | High | High |
...
- _Definition_ An off-centre-fed dipole (OCF) is a half-wave wire antenna fed at a point away from the centre
...
- _Definition_ A development of the Windom/OCF concept incorporating intentional feedline radiation
...
- _Definition_ A hybrid antenna system where the feedline is an intentional and controlled part of the radiating structure
...
- _Balance Concept_ Improved balance is achieved between long arm and (short arm + feedline)
...
- _Common Mode_ Common-mode current onl within the vertical section is part of normal operation and not undesirable
- _Comparison to OCF_ Unlike OCF, does not attempt to suppress feedline radiation
- _Comparison to NCW_ Provides a more directly coupled and better intentional feedline contribution
